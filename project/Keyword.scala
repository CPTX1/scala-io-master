...
case class Keyword(
     section:String, id:String, name:String, depth:Int, shortName:String,
     `type`:String, parent:String, shortSummary:String, keywords:Set[String]) {
  override lazy val toString = """{"section":"%s","id":"%s","name":"%s","depth":%s,"shortName":"%s","type":"%s","parent":"%s","shortSummary":"%s","keywords":"%s"}""".format(section,id,name,depth,shortName,`type`,parent,shortSummary,keywords.mkString(" "))
}
